...
//            pdbData = data.annot;
    ...
    function displayChainInSelect(){
        console.log("search for chain in PDB & nextprot");
        var nxChains = [];
        listChain.forEach(function(l){
            if (l.name === identifier){
                var chains = l.chain.split("/");
                chains.forEach(function(c){
                    var chainId = "Protein:"+c;
                    nxChains.push(chainId);
                    $('#molList').append($("<option></option>").val(chainId).html(chainId));
                })
            }
        })
        return nxChains;
    ...
    function loadBioviz(){
        ...
        listChain = parsePDBData(pdbData);
...
//        var listChains = biovizWidget.bioviz("getMoleculesFromStructure", identifier);
        console.log("identifier");
        console.log(identifier);
        
//        var listChains = biovizWidget.bioviz("getResiduesFromMolecule", identifier);
...
//            biovizWidget.bioviz("highlight", selection, true, null, range, false);
        })
    }
    ...
    function parsePDBData(list3Delems) {
        var listProtChain = list3Delems.map(function (d) {
...
//            var positions = d.targetingIsoformsMap[Object.keys(d.targetingIsoformsMap)[0]];
        ...
        listProtChain.sort(function (a, b) {
