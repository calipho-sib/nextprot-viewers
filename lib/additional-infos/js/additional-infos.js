    ...
    function filterExceptions(typeList) {
        ...
        for (elem in typeList){
            if (typeList[elem].databaseName === "Ensembl"){
                ...
                var suffix = "";
                if (typeList[elem].properties.length>0){
                    var addInfos = $.grep(typeList[elem].properties, function(e){ return e.name === "entry name" || e.name === "isoform ID" || e.name === "gene designation" || e.name === "taxonomic scope" || e.name === "residue range"; })[0];
