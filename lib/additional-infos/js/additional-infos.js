var AdditionalInfos = (function () {
    ...
    AdditionalInfos.prototype.compileKeyword = function (entry,nx, view, div){
        $.getJSON("vendor/featureConfig.json", function (config) {
            nx.getAnnotationsByCategory(entry, "keyword")
                .then(function (data) {
                    var keywordList = getKeywordByview(config[view].keywords, data.annot);
