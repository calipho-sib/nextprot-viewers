$(function () {
    ...
    var loadExternalInfo = function (crossRef, nxEntryName) {
        ...
        function postProcessing(data){
            for (var property in data) {
                ...
                if (data.hasOwnProperty(property)) {
                    if (property.startsWith("EnsemblENST")) {
                        ...
                        if (data[property].additionalInfos.length) {
                            ...
                            console.log(geneID);
                            var isoRef = data["Ensembl"+ isoformID];
                            var geneRef = data["Ensembl" + geneID];
                ...
                if (data[property].dBName === "EMBL") {
                    ...
                    data[property].crossLink[0].preLink = type === "mRNA" ? type : "Translation";
                    ...
                    if (type === "protein") {
                        data[property].crossLink.unshift({
                            ...
                            "preLink": other.name === "genomic sequence ID" ? "Genomic DNA" : other.name
                        });
                    }
                    else if (type === "mRNA"){
                        data[property].crossLink.push({
                            ...
                            "link": mainLink,
                            "resolvedLink": mainLink + other.value,
                            "preLink": other.name === "protein sequence ID" ? "Translation" : other.name
                        });
                    }
                    data[property].additionalInfos = null;
                }
                if (data[property].dBName === "RefSeq") {
                    var nucID = "";
                    data[property].additionalInfos.forEach(function(a) {
                        if (a.name === "nucleotide sequence ID") {
                            nucID = a.value;
                        }
                    });
                    var mainLinkNuc = data[property].crossLink[0].link.slice(0,-10) + "nuccore/" ;
                    data[property].crossLink[0].preLink = "Protein";

                    data[property].crossLink.push({
                        "accession": nucID,
                        "link": mainLinkNuc,
                        "resolvedLink": mainLinkNuc + nucID,
                        "preLink": "Nucleotide Sequence"
                    });
                    data[property].additionalInfos = null;
                }
                //TODO additionalInformation missing for EMBL ref in P01308/sequence
            };
            return data;
        }
        ...
        $.getJSON("../src/featureConfig.json", function (config) {
            console.log(config);
            console.log(config.sequence.crossRef);
            var listParam = config.sequence.crossRef;
            var typeMap = {};
            ...
            crossRef.forEach(function(c) {
                if (typeMap.hasOwnProperty(c.databaseCategory) && listParam[c.databaseCategory].indexOf(c.databaseName) > -1) {
                    var xrefKey = c.databaseName + c.accession;
                    typeMap[c.databaseCategory][xrefKey] = linkParser(c);
                }
            });
    ...
    if ($("#nx-crossRef")) { // load the overview if it exists
        ...
        nx.getXrefs().then(function (data) {
